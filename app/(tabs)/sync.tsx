...
import { PhotoPrismMergedPhoto, SearchPhotos } from "@/scripts/types/photoprism";
import {
  Alert,
  AlertIcon,
  AlertText,
...
export default async function App() {
  ...
  const assetDeleted: PhotoPrismMergedPhoto[] = prismAssets.filter(photo  => 
  ...
  const missingAssets: Asset[] = localAssets.filter(asset =>
  ...
  async function deleteAssets(){
    const deletedPhotosUIDs: string[] = assetsToDelete.map(photo => photo.UID);
  ...
  while (batchTimePerFile < previousBatchTimePerFile && currentBatchSize <= batchSizeLimit) {
    const startTime: number = Date.now();

    const batchAssets: Asset[] = assetsToUpload.slice(indexUploaded, indexUploaded + currentBatchSize);
